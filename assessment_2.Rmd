...
already installed so should run now ^^^


Testing first using the practical information to conduct the work. 
```{r}
...
Shows that the all data was taken from the same platform. You can see that it contains assayData, phenoData, feature etc.
```{r}
...
# careful re-runing this code multple times. It will keep reassigning at raking 1 column from the data!
```
Look at the sample information, gene annotation, and the expression data. This allow us to have a rough idea of the information storing in this expressionSet.
...
```
Check I do in fact have the mean, 3rd quatrile and max values.I do. Can continue. 
```{r}
...
```
INSPECT THE CLIJICAL VARIABLES
```{r}
library(dplyr)
```
...
```
Going to try and see if the tumour type shows some separation (A main priciple component)
```{r}
...
```
Not sure what this is actually showing...The PCA is done on age (0,1) not sur ehat the PCA compents are. 
...
cbind(sampleInfo, pca$x) %>%
  ...
  theme_minimal() +                         # Use a clean theme
  theme(
    legend.position = "right",             # Place the legend on the side
...
When investigatin git's worth noting that ages may not be the culprit for the differential expression it could be due to something else. Going forward with the diferential expression seen betwe nthe 2 groups though. 
...
```{r}
# Here, we assume 'group' is a factor with two levels: 0 (young) and 1 (old)
...
Tell how many are significant. Only dow nreg in old. Which means we actuallu have 15 upregulated in young. All others aren't siginicant. Should be able to look at these and see affects and suggest papers.
```{r}
...
```
see onenote for explanation about the cluster analysis. 
